...
class DashboardServer:
    ...
    def _get_metrics(self) -> Dict:
        ...
        if self.bot and hasattr(self.bot, 'risk_manager'):
            ...
            # Calculate profit factor from closed positions (list of Position objects)
            gross_profit = 0
            gross_loss = 0
            if hasattr(pm, 'closed_positions') and isinstance(pm.closed_positions, list):
    ...
    def _get_trades(self, limit: int = 20) -> List[Dict]:
        """Get trade history."""
        if self.bot and hasattr(self.bot, 'position_manager'):
            trades = []
            if hasattr(self.bot.position_manager, 'closed_positions') and isinstance(self.bot.position_manager.closed_positions, list):
                # closed_positions is a list of Position objects
                for position in self.bot.position_manager.closed_positions[-limit:]:
                    trades.append({
                        'id': f"trade_{position.symbol}_{int(position.entry_time.timestamp())}",
                        'timestamp': position.exit_time.isoformat() if position.exit_time else datetime.now().isoformat(),
                        'symbol': position.symbol,
                        'strategy': 'EMA_SCALP',
                        'side': position.side,
                        'entry_price': position.entry_price,
                        'exit_price': position.exit_price if position.exit_price else 0,
                        'pnl': position.pnl,
                        'pnl_percent': position.pnl_percent,
                        'reason': 'take_profit' if position.pnl > 0 else 'stop_loss'
                    })
            return trades
        else:
            return self.mock_data['trade_history']
    ...
    def _get_positions(self) -> List[Dict]:
        ...
        if self.bot and hasattr(self.bot, 'position_manager'):
            ...
            # Check both open_positions and positions
            open_pos_dict = getattr(self.bot.position_manager, 'open_positions', None)
            if not open_pos_dict:
                open_pos_dict = getattr(self.bot.position_manager, 'positions', {})
            
            if open_pos_dict:
                for symbol, position in open_pos_dict.items():
                    ...
                    if hasattr(position, 'entry_price'):
                        ...
                        positions.append({
                            ...
                            'trailing_active': position.trailing_stop_active if hasattr(position, 'trailing_stop_active') else False,
                            'trailing_stop': position.trailing_stop_price if hasattr(position, 'trailing_stop_price') and position.trailing_stop_price else None,
